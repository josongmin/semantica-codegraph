...
python -m benchmarks.compare \
  ...
  --queries benchmarks/datasets/example_queries.txt \
  --ground-truth benchmarks/datasets/example_ground_truth.json \
  --k 5
```
...
`benchmarks/evaluators/my_retriever.py`:
...
`benchmarks/evaluators/metrics.py`의 `MetricsCalculator`에 메서드 추가:
