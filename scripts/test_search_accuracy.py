...
def setup_test_repos(bootstrap):
    ...
    if python_path.exists():
        ...
        result = bootstrap.pipeline.index_repository(
            root_path=str(python_path), repo_id="search-test-python", name="Search Test Python"
        )
    ...
    if ts_path.exists():
        ...
        result = bootstrap.pipeline.index_repository(
            root_path=str(ts_path), repo_id="search-test-typescript", name="Search Test TypeScript"
        )
        print(f"  ✓ {result.total_files}파일, {result.total_nodes}노드, {result.total_chunks}청크\n")
...
def test_search_case(retriever, chunk_store, repo_id, query, expected_keywords, test_name):
    ...
    results = retriever.retrieve(repo_id=repo_id, query=query, k=5)
...
def run_python_tests(bootstrap, retriever, chunk_store):
    ...
    test_cases = [
        ...
        {"query": "cache store retrieve", "expected": ["Cache", "get", "set"], "name": "캐시 구현"},
        # 고급 패턴
        {
