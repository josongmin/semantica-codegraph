...
from ..core.models import CodeChunk, CodeNode, RepoId, Span
...
class Chunker:
    ...
    def chunk(self, nodes: List[CodeNode]) -> List[CodeChunk]:
        ...
        logger.info(
            f"Chunked {len(nodes)} nodes into {len(chunks)} chunks "
    ...
    def _chunk_hierarchical(self, nodes: List[CodeNode]) -> List[CodeChunk]:
        ...
        # 파일별로 처리
        for file_path, file_nodes_list in file_nodes.items():
            # Class 노드 찾기
