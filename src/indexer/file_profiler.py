...
class FileProfiler:
    ...
    API_PATTERNS = {
        "fastapi": {
            "decorators": [r"@router\.(get|post|put|delete|patch)", r"@app\.(get|post|put|delete|patch)"],
        ...
        "spring": {
            "decorators": [r"@(Get|Post|Put|Delete)Mapping", r"@RestController", r"@RequestMapping"],
    ...
    def profile_file(
        ...
        try:
            ...
            # 5. 함수/클래스 개수 (간단한 정규식)
            profile.function_count = len(re.findall(r"^\s*(def|function|async def)\s+\w+", content, re.MULTILINE))
    ...
    def _tag_by_content(self, profile: FileProfile, content: str, framework: str | None):
        ...
        # Django 패턴
        elif framework == "django" or "from django" in content:
            if "@api_view" in content or "APIView" in content:
                profile.is_api_file = True
        
        # Express 패턴
        elif framework == "express" or "express" in content:
    ...
    def _detect_file_framework(self, content: str) -> str | None:
        ...
        for framework, patterns in self.API_PATTERNS.items():
            ...
            if "imports" in patterns:
                if any(imp in content for imp in patterns["imports"]):
                    return framework
    ...
    def _extract_endpoints(self, content: str, framework: str | None) -> list[dict]:
        ...
        # Express
            ...
            # router.post('/search', ...)
            for match in re.finditer(r'(?:app|router)\.(get|post|put|delete)\(["\']([^"\']+)["\']', content):
                method = match.group(1).upper()
                path = match.group(2)
                endpoints.append({"method": method, "path": path})
