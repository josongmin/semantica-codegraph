...
class RepoProfiler:
    ...
    def profile_repo(self, repo_root: str, repo_id: RepoId) -> RepoProfile:
        ...
        # 3. 프레임워크 감지
        profile.framework, profile.frameworks = self._detect_frameworks(
            repo_path, profile.dependencies
        )
        ...
        logger.info(
            f"[RepoProfiler] 완료: framework={profile.framework}, type={profile.project_type}"
        )
        return profile
    ...
    def _detect_frameworks(
        self, repo_path: Path, dependencies: dict
        ...
        # 1. 의존성 기반 감지
        for framework, _patterns in self.FRAMEWORK_PATTERNS.items():
    ...
    def _detect_api_patterns(self, repo_path: Path, framework: str | None) -> list[str]:
        ...
        api_dirs = [
            ...
            if d.is_dir() and any(kw in d.name.lower() for kw in ["api", "routes", "controllers"])
        ]
