...
class CallPattern:
    ...
    confidence: float
    description: str
    framework: Optional[str] = None  # "django", "flask", None (범용)
...
class PatternAnalyzer:
    ...
    def _select_patterns(self) -> List[CallPattern]:
        ...
        for pattern in self.PATTERNS:
            # 범용 패턴은 항상 포함
            if pattern.framework is None:
                active.append(pattern)
            # 프레임워크 특화 패턴
            elif pattern.framework == self.framework:
                active.append(pattern)
    ...
    def _extract_node_code(self, code: str, node: ast.AST) -> Optional[str]:
        """AST 노드의 소스 코드 추출"""
        try:
            # Python 3.9+
    ...
    def _match_patterns(
        ...
        code: str,
        node: ast.AST,
        available_symbols: Optional[Set[str]]
    ) -> List[PatternMatch]:
        """코드에서 패턴 매칭"""
        matches = []
