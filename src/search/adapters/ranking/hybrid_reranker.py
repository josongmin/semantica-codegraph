"""하이브리드 리랭커 (Query Type + Importance Boost)"""
...
class HybridReranker:
    ...
    def rerank(
        ...
        # OpenTelemetry span
        if tracer:
            with tracer.start_as_current_span("hybrid_reranking") as span:
                try:
                    span.set_attribute("query", query[:100])
                    span.set_attribute("candidates.count", len(candidates))
                    span.set_attribute("max_items", max_items)

                    result = self._execute_rerank(query, candidates, max_items, span)

                    span.set_attribute("results.count", len(result))
                    if result:
                        span.set_attribute("results.top_score", result[0].features.get("final_score", 0))
            ...
            return self._execute_rerank(query, candidates, max_items, None)

    def _execute_rerank(
        self, query: str, candidates: list[Candidate], max_items: int, span: Any
    ) -> list[Candidate]:
        """Reranking 실행"""
        # 1. 쿼리 타입 분류
        ...
        # 4. 상위 max_items개 반환
        result = reranked[:max_items]

        logger.info(
            f"Reranked {len(candidates)} candidates, returning top {len(result)} "
